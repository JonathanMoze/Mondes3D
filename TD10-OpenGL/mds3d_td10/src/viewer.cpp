...
void Viewer::drawArticulatedArm() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    M *= AngleAxisf(_jointAngles(0, i), Vector3f::UnitZ());
    setObjectMatrix(_shader, (M * Scaling(jointScale)).matrix());
    glUniform3fv(_shader.getUniformLocation("color"), 1,
                 Vector3f(0.8f, 0.4f, 0.4f).data());
    _jointMesh.draw(_shader);

    M *= AngleAxisf(_jointAngles(1, i), Vector3f::UnitY());

    setObjectMatrix(_shader, (M * Scaling(Vector3f(1, 1, _lengths(i)))).matrix());
    glUniform3fv(_shader.getUniformLocation("color"), 1,
                 Vector3f(0.8f, 0.8f, 0.4f).data());
    _segmentMesh.draw(_shader);
    _mats[i] = M.matrix() * Bj.matrix().inverse();
    Bj *= Translation3f(Vector3f(0, 0, _lengths(i)));
    M *= Translation3f(Vector3f(0, 0, _lengths(i)));
...
void Viewer::drawCylinder() {
  ...
  glUniform3fv(_cylinderShader.getUniformLocation("lightDir"), 1,
               lightDir.data());
  glUniform3fv(_cylinderShader.getUniformLocation("lengths"),  1, _lengths.data());
  glUniformMatrix4fv(_cylinderShader.getUniformLocation("MJs"), 3, GL_FALSE, _mats[0].data());
...
void Viewer::updateAndDrawScene() {
  if (_IK_target.norm() > 0) {
   ...
   Affine3f M;
    M.setIdentity();
    int n = int(_lengths.size());
    for (int i = 0; i < n; i++) {
      M *= AngleAxisf(_jointAngles(0, i), Vector3f::UnitZ());
      M *= AngleAxisf(_jointAngles(1, i), Vector3f::UnitY());
      M *= Translation3f(Vector3f(0, 0, _lengths(i)));
    }
    Vector3f position = M.translation();
    M.setIdentity();
    Eigen::MatrixXf jacob;
    jacob.resize(3, 2*n);
    for(int i = 0; i <n; i++) {
        Vector3f posJoint = M.translation();
        M = M * AngleAxisf(_jointAngles.col(i).x(), Vector3f::UnitZ());
        Vector3f derivated1 = M.linear().col(2).cross(position - posJoint);
        jacob.col(i*2) = derivated1;

        M = M * AngleAxisf(_jointAngles.col(i).y(), Vector3f::UnitY());
        Vector3f derivated2 =  M.linear().col(1).cross(position - posJoint);
        jacob.col((i*2)+1) = derivated2; 

        M = M * Translation3f(0, 0, _lengths(i));
    }
    MatrixXf Gradient = jacob.transpose() * (_IK_target-position);
    Gradient.transpose();

    for(int j=0; j<3;j++){
      _jointAngles.col(j)[0] += 0.001 * M_PI * Gradient.col(0)[(j*2)]; 
      _jointAngles.col(j)[1] += 0.001 * M_PI * Gradient.col(0)[(j*2)+1];
    }
  }
  drawScene();
  drawCylinder();
